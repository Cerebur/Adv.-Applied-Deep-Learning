...
model_name='fixed_beta'  # name of the model
...
def sample_reverse(g, count, samples_evolution_num=10):
    """
    ...
    # create a tensor of reverse order time steps in the shape of (count, TIME_STEPS)
    t = torch.arange(TIME_STEPS, 0, -1).repeat(count, 1).to(device)  # shape (count, TIME_STEPS)
    ...
    for i in range(TIME_STEPS-1, -1, -1):
        ...
        sqrt_one_minus_alpha_bar_t = torch.sqrt(1 - alpha_bar[i]).to(device)
        # sample a random noise from the standard normal distribution
        z = torch.randn(count, 1).to(device) if i > 0 else 0
        # compute the new sample
        x = 1 / torch.sqrt(alpha[i]) * (x - (1 - alpha[i]) / sqrt_one_minus_alpha_bar_t * g(x, t[:, i])) + z * torch.sqrt(beta_schedule[i])
        # save the evolution of the samples
        x_evolution[:, i] = x[indices].squeeze(1)  # save the current samples
    return x, x_evolution


samples, samples_evolution = sample_reverse(model, 1000, samples_evolution_num=50)
...
for i in range(len(samples_evolution)):
    ax.plot(torch.arange(TIME_STEPS, 0, -1),samples_evolution[i].detach().cpu().numpy(), label=f'Sample {i+1}', color='xkcd:gray', alpha=0.5)
